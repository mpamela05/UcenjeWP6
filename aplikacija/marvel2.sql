DROP database if EXISTS marvel2;

create database marvel2;
...
tim int ,
primary key (sifra),
foreign key (identitet) references identitet(sifra),
foreign key (tim) references timovi(sifra)
);

insert into identitet (ime, prezime, godine, god_rodjenja, god_smrti)
